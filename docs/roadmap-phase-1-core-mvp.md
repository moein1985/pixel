...
- [x] احراز هویت OTP پیامکی کار می‌کند (Redis + DB fallback، cooldown، attempts)
- [x] ثبت‌نام با ۴ نقش (farmer, supplier, company, moderator) کار می‌کند
- [x] پروفایل کشاورز با تمام فیلدها قابل ایجاد و ویرایش است
- [x] پروفایل شرکت با تمام فیلدها قابل ایجاد و ویرایش است
- [x] پروفایل تأمین‌کننده با تمام فیلدها قابل ایجاد و ویرایش است
- [ ] جستجوی فارسی با pg_trgm کار می‌کند (بدون نیم‌فاصله) — فعلاً ilike، نیاز به migration برای indexهای trgm
- [x] فیلتر بر اساس استان، نوع، دسته‌بندی کار می‌کند
- [x] پنل مدیریت کار می‌کند (تأیید پروفایل، مدیریت کاربران)
- [x] آپلود تصویر و فایل به MinIO کار می‌کند (avatar, logo, document, image)
- [x] صفحه اصلی پورتال نمایش داده می‌شود
- [x] تمام unit testها pass می‌شوند (Vitest: ۱۳/۱۳) — E2E Playwright نوشته شده اما نیاز به سرور اجرایی
